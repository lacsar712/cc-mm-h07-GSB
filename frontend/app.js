...
function paint(list) {
  rows.innerHTML = list
    .map(
      (r) =>
        `<tr data-id="${r.id}"><td><a href="#" data-open="${r.id}">${r.site}</a></td><td>${r.ch4_pct}</td><td class="${r.level === "报警" ? "alarm" : "ok"}">${r.level}</td><td>${r.note}</td></tr>`,
    )
    .join("");
  rows.querySelectorAll("[data-open]").forEach((a) => {
    a.onclick = async (ev) => {
      ev.preventDefault();
      const data = await api(`/api/readings/${a.dataset.open}`);
      if (detailBox) {
        detailBox.textContent = `${data.site} · ${data.level} · 浓度 ${data.ch4_display || data.ch4_pct}（${data.ch4_blurb || ""}）`;
      }
    };
  });
}
